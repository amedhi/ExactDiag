...
public:
  ...
  unsigned add_siteterm(const std::string& name, const CouplingConstant& cc, const TermOp& op);
  unsigned add_bondterm(const std::string& name, const CouplingConstant& cc, const TermOp& op);
...
private:
  ...
  // operetaors
  std::vector<TermOp> op_Sx_;
  std::vector<TermOp> op_Sy_;
  std::vector<TermOp> op_Sz_;
  std::vector<TermOp> op_SmSp_;
